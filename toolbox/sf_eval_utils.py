...
import numpy as np
from toolbox.utils import _EPS, save_pkl, load_pkl
...
def compute_sf_metrics_numpy(epe_per_point, relative_error):
...
def collect_scene_stats(epe_per_point, relative_error, sd_label, fb_label):
    metrics = dict()
    metrics['moving_ratio'] = sd_label.float().mean().item()
    metrics['FG_ratio'] = fb_label.float().mean().item() 
    gt_mag = epe_per_point / (relative_error + 1e-20)
    if sd_label.sum():
        dynamic_mask = sd_label == 1
        metrics['Dynamic'] = compute_sf_metrics_torch(epe_per_point[dynamic_mask], relative_error[dynamic_mask])
        metrics['Dynamic_motion_mag'] = gt_mag[dynamic_mask].mean().item()
    static_mask = fb_label == 1
    metrics['Static'] = compute_sf_metrics_torch(epe_per_point[static_mask], relative_error[static_mask])
...
class SF_Evaluator():
    ...
    def update(self, gt_flow, est_flow, time_indice, fb_label, sd_label, mask = None, relative_error= None, epe_per_point =None):
        if mask is not None:
            est_flow, gt_flow = est_flow[mask], gt_flow[mask]
            fb_label, sd_label = fb_label[mask], sd_label[mask]
            
            if relative_error is not None:
                relative_error, epe_per_point = relative_error[mask], epe_per_point[mask]
        
        if relative_error is None:
            error = est_flow - gt_flow
            epe_per_point = torch.norm(error, p=2, dim=1)
            gt_f_magnitude = torch.norm(gt_flow, p=2, dim=1)
            relative_error = epe_per_point / (gt_f_magnitude + _EPS)

        self.flow_error_list.extend(epe_per_point.cpu().numpy().astype(np.float16))
        self.relative_errro_list.extend(relative_error.cpu().numpy().astype(np.float16))
    ...
    def sf_evaluate(self, fb_label, sd_label, epe_per_point, relative_err):
        ...
        metrics['overall'] = compute_sf_metrics_numpy(epe_per_point,relative_err)

        bckg_mask = (fb_label == 0)
        metrics['BG'] = compute_sf_metrics_numpy(epe_per_point[bckg_mask],relative_err[bckg_mask])
        
        forg_mask = (fb_label == 1)
        metrics['FG'] =compute_sf_metrics_numpy(epe_per_point[forg_mask],relative_err[forg_mask])
        
        static_mask = sd_label == 0
        metrics['Static'] = compute_sf_metrics_numpy(epe_per_point[static_mask], relative_err[static_mask])
        
        dynamic_mask = sd_label == 1
        metrics['Dynamic'] = compute_sf_metrics_numpy(epe_per_point[dynamic_mask], relative_err[dynamic_mask])
