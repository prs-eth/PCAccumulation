...
from toolbox.utils import Logger,validate_gradient, _EPS, load_yaml, partial_load
...
class BaseTrainer(object):
    ...
    def _load_pretrain(self, resume):
        ...
        if os.path.isfile(resume):
            ...
            basedir = os.path.dirname(resume)
            tffiles = glob.glob(f'{basedir}/tensorboard/*')
            for eachfile in tffiles:
                os.system(f"cp {eachfile} {self.config['tboard_dir']}")
    ...
    def update_tensorboard(self, stats_meter, curr_iter, phase):
        ...
        if 'fb_metric' in stats_meter.keys():
    ...
    def inference_one_batch(self, input_dict, phase):
        ...
        if(phase == 'train'):
            ...
            loss = stats['loss'] / self.iter_size
            loss.backward()
        else:
            self.model.eval()
            with torch.no_grad():
    ...
    def inference_one_epoch(self, epoch, phase):
        ...
        for c_iter in tqdm(range(num_iter)): # loop through this epoch   
            ...
            ################################
            # update to the wandb/tensorboard
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.max_epoch):
            ...
            new_stats, message = compute_mean_iou_recall_precision(stats_meter['mos_metric'], self.mos_mapping)
