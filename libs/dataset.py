...
from toolbox.register_utils import apply_tsfm
...
class BaseDataset(Dataset):
    ...
    def prep_input(self, raw_points, sd_labels, fb_labels, inst_labels, time_indice, ego_motion_gt, inst_motion_gt):
        ...
        # 4. convert to point pillars
        points = np.concatenate((raw_points, time_indice[:,None]), axis=1).astype(np.float32)
        voxels = self.voxeliser(points)
        num_points = np.array([raw_points.shape[0]], dtype=np.int64)
...
class NuSceneDataset(BaseDataset):
    DATA_FILES = {
        'train': 'assets/configs/datasets/nuscene/train_info.txt',
        'val': 'assets/configs/datasets/nuscene/val_info.txt',
        'test': 'assets/configs/datasets/nuscene/test_info.txt',
    } 

class WaymoDataset(BaseDataset):
    DATA_FILES = {
        'train': 'assets/configs/datasets/waymo/train_info.txt',
        'val': 'assets/configs/datasets/waymo/val_info.txt',
        'test': 'assets/configs/datasets/waymo/test_info.txt',
    } 