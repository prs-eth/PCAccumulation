...
from utils import load_pkl, natural_key, to_o3d_pcd, multi_vis, setup_seed
import numpy as np
...
if __name__=='__main__':
    ...
    for seq_id in range(20):
        ...
        lidar_path.extend(sorted(glob(data_dir+f'/{seq_id}/lidar/*.npy'), key=natural_key))
        label_path.extend(sorted(glob(data_dir+f'/{seq_id}/label/*.pkl'), key=natural_key))
    ...
    for idx, lidar in tqdm(enumerate(lidar_path)):
        ...
        is_not_ground_1 = get_ground(points)
        pcd_remove_icra = to_o3d_pcd(points[is_not_ground_1])
        ...
        is_not_ground_2 = points[:,2] > ground_height+0.05
        pcd_remove_thre = to_o3d_pcd(points[is_not_ground_2])

        pcd_remove_both = to_o3d_pcd(points[np.logical_and(is_not_ground_1, is_not_ground_2)])
        multi_vis([pcd, pcd_remove_icra, pcd_remove_thre, pcd_remove_both],['input','GPF','Simple thresholding','Use both'])